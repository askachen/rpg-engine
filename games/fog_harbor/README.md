...
拾取鑰匙、調查舊置物櫃後找瑪拉閒聊，選「看看投影機測試片」。先播一次，再循環；可用滑鼠暫停、改影片音量、跳過或取消。短片為有聲 4:3 技術測試圖，非正式劇情 CG。見 [影片契約](../../docs/video.md)。

## S4-08 畫廊

完成伊莉絲路線後解鎖港口 CG，完成瑪拉路線後解鎖投影機技術測試片。從開頭／遊戲選單進入畫廊，可分類、放大、播放、暫停與返回；新遊戲仍保留解鎖。測試片並非正式故事影片。

## S4-09 背包／商店

拾取鑰匙後靠近舊置物櫃，打開背包可直接使用（消耗一把）。工坊商品會顯示庫存、持有量及缺錢／售罄原因；碼頭津貼可提供購買資金。角色進度可收合；選單新增製作名單及離開確認。
