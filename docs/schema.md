...
Schema 不證明所有任務可達，也不證明圖片、音訊、影片可解碼或字型包含所有字。引用、碰撞及必要依賴循環仍由語意 validator 處理，素材／語系深化屬 S2-03。存檔檔案自身的 Schema 與遷移仍是 S6-01，initial 的結構規格不能代替存檔驗收。

Python CLI 會先驗證再啟動；直接從 Godot 編輯器／執行檔載入內容時，不會內嵌 Python jsonschema。發布前的強制驗證與打包流程屬 S7，不以本機編輯器能開啟表示內容有效。
