extends RefCounted
## Read-only rendering adapter. Called exclusively within the host Control._draw.
func draw(view: Control) -> void:
	...
	view.draw_style_box(frame, bounds.grow(8))
	...
	for y in range(area.height):
		for x in range(area.width):
			...
			view.art.material(view, floor_sheet, floor_index, Rect2(view.ORIGIN + Vector2(x, y) * unit, Vector2(unit, unit)), Vector2i(x, y), repeats, tint)
	...
	for wall in area.walls:
		...
		view.draw_rect(Rect2(at + Vector2(2, 5), Vector2(unit, unit)), Color(0, 0, 0, 0.18))
		var wall_spec: Dictionary = view.appearance.spec.get("world", {}).get("wall", {})
		if not wall_spec.is_empty():
			view.art.draw(view, wall_spec.sheet, int(wall_spec.vertical if vertical else wall_spec.horizontal), Rect2(at, Vector2(unit, unit)), Color("f2e4cd"), false)
		if vertical: view.draw_line(at + Vector2(2, 0), at + Vector2(2, unit), Color("7d6850"), 3)
		else: view.draw_line(at + Vector2(0, 2), at + Vector2(unit, 2), Color("7d6850"), 3)
	for decor in area.get("decorations", []):
	...
	for cell in view.walking:
		view.draw_circle(view.ORIGIN + (Vector2(cell) + Vector2(0.5, 0.5)) * unit, 3, Color(1, 0.94, 0.7, 0.55))
	if view.core.state.period == "late": view.draw_rect(bounds, Color(0.10, 0.15, 0.27, 0.18))
	elif view.core.state.period == "evening": view.draw_rect(bounds, Color(0.55, 0.28, 0.1, 0.06))

func draw_prop(view: Control, prop: Dictionary, unit: float) -> void:
	view.art.draw(view, prop.get("sheet", ""), int(prop.sprite), Rect2(view.ORIGIN + Vector2(prop.position[0], prop.position[1]) * unit, Vector2(prop.size[0], prop.size[1]) * unit))
...
func draw_actor(view: Control, who: String, position_value: Array, unit: float, moving: bool = false) -> void:
	...
	view.draw_style_box(shadow, Rect2(feet + Vector2(-unit * 0.27, -5), Vector2(unit * 0.54, 10)))
	view.art.draw_fitted(view, view.art.resolve(view.core.content.avatars[who].sprite), Rect2(feet + Vector2(-unit * 0.65, -unit * 1.85 + bob), Vector2(unit * 1.3, unit * 1.85)))
...
func draw_chip(view: Control, text: String, center: Vector2, tint: Color = Color("efe3c8")) -> void:
	...
	var at = Vector2(clampf(center.x - text_size.x / 2.0, view.ORIGIN.x + 4, 1370 - text_size.x), center.y)
	var style = tile_style(view, Color(0.12, 0.19, 0.15, 0.87))
	style.set_corner_radius_all(6)
	view.draw_style_box(style, Rect2(at + Vector2(-7, -20), text_size + Vector2(14, 6)))
	view.draw_string(view.appearance.font, at, text, HORIZONTAL_ALIGNMENT_LEFT, -1, 17, tint)
...
func draw_icon(view: Control, kind: String, rect: Rect2) -> void:
	...
	if spec != null: view.art.draw_fitted(view, view.art.resolve(spec), rect)
