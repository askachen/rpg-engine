...
**目前適合技術評估與原型開發，尚未達到正式對外交付／Steam 發行品質。** S2 工具鏈已確認；S3 已確認；S4-01 地圖視窗／轉場已實作，面向與動畫仍開發中；完整狀態以 [TASKS.md](TASKS.md) 為準。規劃中的功能不代表已實作。
...
- 多段對話、選項、打字效果、自動播放、已讀快轉、紀錄、背景／肖像圖片及音訊。
- 開頭選單、基本設定、中英切換、6 個手動存檔槽、自動存檔、繼續遊戲、紀念卡與開發者狀態畫面。
...
大地圖現在會跟隨玩家捲動並裁切於固定世界視窗，HUD 不移動；滑鼠提示顯示互動物件名稱，出入口有淡出／淡入與輸入鎖定。霧港工坊地毯已改為明確地面層，可放在桌子與角色底下。使用 `python tools/dev.py play --game fog_harbor` 試玩；鏡頭、素材層級與測試契約見 [地圖視窗說明](docs/world-view.md)。四方向行走動畫尚未完成，S4-01 保持開發中。
